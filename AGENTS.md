...
| Stored | AWS Secrets Manager (scoped IAM per consumer) | `platform-bootstrap/github-app-pem`; `platform-bootstrap/tfe-api-token` (HCP org API); `personal/cloudflare-api-token` (DNS, 5 zones) — **factory / HCP plan-time only**. Homelab runtime + comms tokens → **Vault**; operator copies → **Apple Password** (see runbook 10). Legacy workstation MCP tokens in SM (`personal/linear-api-token`, etc.) are being retired — see runbooks 08–10 |
