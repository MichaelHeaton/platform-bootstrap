...
- **Domain spokes read SM at plan time**: the `cloudflare` repo (`SpecterRealm/cloudflare`) reads `personal/cloudflare-api-token` via `data.aws_secretsmanager_secret_version`. New infra spokes belong in an org (`specterrealm_repositories`) so the GitHub App can create them; set `github_org` on the pipeline entry for OIDC trust — see runbook 09.
